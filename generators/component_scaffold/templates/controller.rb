class <%= controller_class_name %>Controller < ApplicationController
  ...
  def create
    @<%= file_name %> = <%= controller_class_name %>.new(params[:components_<%= file_name %>])

    respond_to do |format|
      if @<%= file_name %>.save
        flash[:notice] = '<%= class_name %> was successfully created.'
        format.html { redirect_to(components_<%= table_name %>_url) }
  ...
  def update
    ...
    respond_to do |format|
      if @<%= file_name %>.update_attributes(params[:components_<%= file_name %>])
